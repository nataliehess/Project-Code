...
def on_message(client, obj, msg):
    ...
    print(json_data)
    door_set = json.loads(json_data)['washer'][0]['door_set']
    temp_set = json.loads(json_data)['washer'][1]['temp_set']
    preheat_set = json.loads(json_data)['washer'][0]['preheat_set']
    if door_set == 'on':
        door_set = GPIO.HIGH
...
# device topics
in_oven_preheat_set  = 'washer/preheat_set' + device_id + '/get'   # receiving messages
in_oven_temp_set     = 'washer/temp_set' + device_id + '/get'      # receiving messages
in_oven_door_set     = 'washer/door_set' + device_id + '/get'      # receiving messages
out_oven_door_stat   = 'washer/door_stat' + device_id + '/set'     # publishing messages
out_oven_cur_stat    = 'washer/cur_stat' + device_id + '/set'      # publishing messages
out_oven_cur_temp    = 'washer/cur_temp' + device_id + '/set'      # publishing messages
...
client.subscribe('washer/#/get', 1)                     # MQTT subscribtion (with QoS level 0)
   ...
   # cur_temp    = RC_Analog(cur_temp)
    if door_status != prev_door_stat and time.time() - updated_at > debounce:
        ...
        if door_status:
            ...
            payload_door = { 'washer': [{ 'id': '518be5a700045e1521000001', 'door_status': payload_door }] }
    if cur_status != prev_cur_stat and time.time() - updated_at > debounce:
        ...
        if cur_status:
            ...
            # effectively update the light status
            GPIO.output(door_stat, door_status)
            payload_cur = { 'washer': [{ 'id': '', 'cur_status': payload_cur }] }
            #payload_temp = { 'washer': [{ 'id': '', 'cur_temp': cur_temp }] }
